...
- **Amazon EventBridge** — Bus sự kiện serverless định tuyến sự kiện S3 tới Lambda
...
4. **Amazon EventBridge** định tuyến sự kiện `Object Created` tới Lambda khi có file mới vào quarantine bucket.
5. **GuardDuty Malware Protection** quét file và gắn tag `CLEAN` hoặc `THREAT_FOUND`.
6. Một **Lambda** worker được kích hoạt bởi EventBridge đọc tag kết quả quét GuardDuty, di chuyển file sạch sang **DocumentsBucket** và ghi metadata + bản ghi version vào **DynamoDB**.
...
3. [Triển khai hạ tầng bằng AWS CDK](5.3-deploy-infrastructure/)
4. [Cấu hình Backend và kiểm thử API](5.4-test-apis/)
5. [Bảo mật & Giám sát với GuardDuty và CloudWatch](5.5-security-monitoring/)
